...
プロンプトの実行には常に sub agent を立ち上げること
...
6. **t-wada の TDD スタイル**: Red-Green-Refactor サイクルで実装

### 純粋関数と関数型プログラミング

メソッドは純粋関数となるように定義し、関数型プログラミングのスタイルでコーディングを行う。
...
**可読性の原則**: 関数型スタイルが可読性を著しく低下させる場合は、手続き的な記述を許可する

## CLI ツール

以下の CLI ツールを活用:
- `gh`: GitHub CLI（Issue、PR、API 操作）
- `jq`: JSON 処理
- `tree`: ディレクトリ構造表示
...
以下の入力形式を活用:
- `@file` / `@folder`: ファイル・フォルダ参照
- 画像・スクリーンショット: ドラッグ＆ドロップまたはパス指定
- URL: Web ページの内容を取得（WebFetch）
- パイプ: `cat file.json | claude` でデータを直接入力
